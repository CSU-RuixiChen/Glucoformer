...
class ETSA_Layer(nn.Module):
    '''
    The Enhanced Two Stage Attention (ETSA) Layer
    input/output shape: [batch_size, Data_dim(D), Seg_num(L), d_model]
    '''
    def __init__(self, attn1, attn2, seg_num, factor, d_model, n_heads, d_ff = None, dropout=0.1):
        super(ETSA_Layer, self).__init__()
        ...
        # [Glucoformer Upgrade] the router is now conditioned on segment context instead of being static only.
        self.router_bank = nn.Parameter(torch.randn(seg_num, factor, d_model))
    ...
    def forward(self, x):
        ...
        segment_context = dim_send.mean(dim=1)
        segment_context = self.router_context(segment_context).unsqueeze(1)
        batch_router = repeat(self.router_bank, 'seg_num factor d_model -> (repeat seg_num) factor d_model', repeat = batch)
        batch_router = batch_router + segment_context
