...
import os
import argparse
from Crossformer.cross_models.cross_former import *
    ...
    for seed in Seeds:
        ...
        for pred_len_raw in prediction_task:
            ...
            model = Crossformer(data_dim = config.data_dim, in_len = seq_len, out_len = pred_len, seg_len = seg_len,  output_size = config.c_out,
                            factor=config.factor, d_model=config.d_model, d_ff=config.d_ff, n_heads=config.n_heads, e_layers=config.e_layers, dropout=config.dropout)
